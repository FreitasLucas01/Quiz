...
function loadQuestion() {
  ...
  let choices = shuffleArray(questionIndex.choices);
...
function checkAnswer(e) {
  ...
  if (e.target.innerText === questions[currentQuestion].answer) {
    ...
    alert(
      `Errado! A resposta correta era ${questions[currentQuestion].answer}`
    );
  }
}

choiceElements.forEach((itens) => {
  itens.addEventListener("click", checkAnswer);
});

nextButton.addEventListener("click", () => {
  if (!answerChosen) {
    alert("Porn favor, responda a pergunta!");
    return;
  }
